...
from flask import Flask, g, jsonify, render_template, redirect, request, session, url_for
...
## this is the new code where i'm tyring to get manual facebook login flow setup
## this is the new code where i'm tyring to get manual facebook login flow setup
@app.before_request
def get_current_user():
    ...
    # Attempt to get the short term access token for the current user from cookie.
    result = get_user_from_cookie(cookies=request.cookies, app_id=FB_APP_ID, app_secret=FB_APP_SECRET)

     # If there is no result, we assume the user is not logged in.
    if result:
        # Check to see if this user is already in our database.
        user = UserModel.query.filter(UserModel.FBuserID == result['uid']).first() ## this assumes the cookie result uid = FBuserID in our UserModel

        if not user: #new user, not an existing user
            graph = GraphAPI(result['access_token'])
            profile = graph.get_object('me')
            if 'link' not in profile:
                profile['link'] = ""

            user = UserModel(username=None, email=None, password=None, FBuserID=profile['id'], FBAccessToken=result['access_token'])
            db.session.add(user)
        elif user.FBAccessToken != result['access_token']:
             # If an existing user, update the access token
            user.FBAccessToken = result['access_token']

        # add the user to current session /// this is new for me
        session['user'] = dict(FBuserID=user.FBuserID, FBAccessToken=user.FBAccessToken)
    db.session.commit()
