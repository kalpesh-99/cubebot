...
def getRecentReviews(current_user, page=1):
    ...
    if userID:
        ...
        qDistinctContent = db.session.query(ReviewsModel.id.label('reviewNumber'), ReviewsModel.rateValue, ReviewsModel.reviewsOn_contentID, func.round(func.avg(ReviewsModel.rateValue),1).label('average'), func.count(ReviewsModel.rateValue), func.sum(ReviewsModel.rateValue), ContentModel.id.label('cID'), ContentModel.title.label('titleContent'), ContentModel.category, ContentModel.url.label('contentURL'), ContentModel.urlImage.label('contentiURL'), ContentModel.source).join(ContentModel).filter(ReviewsModel.reviewsOn_userID == userID).group_by(ReviewsModel.id, ReviewsModel.reviewsOn_contentID, ContentModel.id).order_by(ReviewsModel.id.desc())
