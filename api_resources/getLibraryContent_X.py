...
def getLibraryContent(contentIDList):
    ...
    userContentQuery = db.session.query(ContentModel.id, ContentModel.title, ContentModel.url, ContentModel.urlImage).filter(ContentModel.id.in_(userContentForThisThread)).order_by(ContentModel.id.desc()).paginate(page, per_page, False)
