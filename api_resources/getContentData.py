...
def getThought(thoughtID, current_user, page=1):
    ...
    userContentIdList = []
    for item in getContentIDquery:
        userContentIdList.append(item[0])
    print(userContentIdList, "this should be the list of ThreadContent ids")

    # userContentIdList = getContentIDquery[0]
    # print(userContentIdList, "is this user content id list?")

    userThoughtContentSelector = thoughtID ## select which content.id we want to use for this thoughts, in this case the most recent content
    print(userThoughtContentSelector, "is this user thought content selector = thought id in url ")
    # userThoughtContentID = userContentIdList[userThoughtContentSelector]
    # print(userThoughtContentID, "is this user thought content id being picked ")

    # userThoughtContentQuery = userContentQuery.filter(ContentModel.id == userThoughtContentID).paginate(page, per_page, False)
    userThoughtContentQuery = userContentQuery.filter(ContentModel.id == userThoughtContentSelector).paginate(page, per_page, False)
